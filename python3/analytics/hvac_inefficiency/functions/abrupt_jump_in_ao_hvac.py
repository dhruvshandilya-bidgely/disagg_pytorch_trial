...
Date - 19th Feb 2021
This file contains code for finding abrupt change in AO HVAC
"""
...
def ao_outlier_basic_function(main_input_hvac, main_output_hvac, hvac_outlier_limit, inlier_deviation=2.,
                              column_number=0, offset=0.20, random_state=43):
    """
    ...
    # Defining quadrant level outliers #
    ...
    try:
        ...
        return_dictionary = {'inliers': [main_input_hvac, main_output_hvac],
                             ...
                             'status': 'First RANSAC fit failed'}
    ...
    try:
        ...
        return_dictionary = {'inliers': [main_input_hvac, main_output_hvac],
                             ...
                             'status': 'Second RANSAC fit failed'}
...
def abrupt_jump_in_ao_hvac(input_hvac_inefficiency_object, output_hvac_inefficiency_object, logger_pass, device):
    ...
    row_idx = input_hvac_inefficiency_object.get(device).get('ao_hvac_pivot').get('row')
    column_idx = input_hvac_inefficiency_object.get(device).get('ao_hvac_pivot').get('columns')
    ...
    minimum_ao_hvac_hours = static_params.get('ineff').get('minimum_ao_hvac_hours')

    # Replace NaNs by zero

    hvac_consumption_matrix[np.isnan(hvac_consumption_matrix)] = 0

    less_hvac_ao = np.count_nonzero(hvac_consumption_matrix, axis=1)
    less_hvac_ao = less_hvac_ao < minimum_ao_hvac_hours
    hvac_consumption_matrix[less_hvac_ao,  :] = 0
    ...
    hvac_potential_hour_count = np.true_divide(hvac_potential_matrix.sum(axis=1),
                                               (hvac_potential_matrix != 0).sum(axis=1),
                                               where=((hvac_potential_matrix != 0).sum(axis=1) != 0))

    hvac_potential_hour_count = np.round(hvac_potential_hour_count, 2)

    # Leave temeprature as it is
    ...
    valid_idx = ((hvac_hour_count > 0) & (unconsidered_hvac_count == 0) &
                 (hvac_potential_hour_count > 0) & (hvac_potential_hour_count != 1))
    ...
    main_input_hvac =\
        np.c_[hvac_potential_hour_count[valid_idx], hvac_temperature_average[valid_idx], date_index[valid_idx]]
    ...
    if not (main_input_hvac.shape[0] == 0):
        return_dictionary = ao_outlier_basic_function(main_input_hvac, main_output_hvac,
                                                      hvac_outlier_limit=hvac_outlier_limit_list[0],
                                                      inlier_deviation=3, column_number=column_number, offset=0.20,
    ...
    if not (main_input_hvac.shape[0] == 0):
        return_dictionary_zero_saturation = ao_outlier_basic_function(main_input_hvac, main_output_hvac,
                                                                      hvac_outlier_limit=hvac_outlier_limit_list[1],
                                                                      inlier_deviation=0.5, column_number=column_number,
                                                                      offset=0.20, random_state=random_state)
    ...
    main_input_hvac =\
        np.c_[hvac_potential_hour_count[valid_idx_saturation], hvac_temperature_average[valid_idx_saturation],
              date_index[valid_idx_saturation]]
    ...
    if not (main_input_hvac.shape[0] == 0):
        return_dictionary_saturation = ao_outlier_basic_function(main_input_hvac, main_output_hvac,
    ...
    string_for_heading = '{} outliers count: {}, (high: {}, low: {})'.format(device, total_outliers, high_outliers,
                                                                             low_outliers)
    abrupt_ao_hvac = {'string': string_for_heading,
    ...
    output_hvac_inefficiency_object[device]['abrupt_ao_hvac'] = abrupt_ao_hvac
