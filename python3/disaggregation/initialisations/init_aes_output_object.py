...
def init_disagg_aes_output_object(disagg_input_object):
    ...
    num_columns = num_modules + 1 + int('hvac_smb' in module_seq) * 7
    ...
    # SMB : 6 Outputs specific to SMBs
    if config.get('user_type').lower() == 'smb':
        smb_specific_output = ['x-ao', 'ac_open', 'ac_close', 'sh_open', 'sh_close', 'op']
        for component in smb_specific_output:
            disagg_output_object['output_write_idx_map'][component] = write_idx
            write_idx += 1
    ...
    lifestyle_data_range_start = disagg_input_object.get('out_bill_cycles_by_module').get('lifestyle_profile')[0][0]
    lifestyle_data_range_end = disagg_input_object.get('out_bill_cycles_by_module').get('lifestyle_profile')[-1][-1]
    ...
    for billcycle_start, billcycle_end in out_bill_cycles:
        ...
        disagg_output_object['lifestyle_profile'][int(billcycle_start)]['profileList'][0]['start'] = \
            int(billcycle_start)
        disagg_output_object['lifestyle_profile'][int(billcycle_start)]['profileList'][0]['end'] = int(
            billcycle_end)
