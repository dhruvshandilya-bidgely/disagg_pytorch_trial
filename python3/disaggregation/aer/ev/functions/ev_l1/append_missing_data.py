...
def append_missing_data(debug):
    ...
    if missing_data.shape[0] > 0:
        ...
        box_data_keys = [key for key in debug.get('l1', {}).keys() if 'box_data' in key]

        for data_key in box_data_keys:
            temp_box_data = debug['l1'].get(data_key)

            n_columns = temp_box_data.shape[1]

            # Add additional columns to box data if the number of columns in box data is different from that of
            # missing data
            ...
            debug['l1'][data_key] = temp_box_data
