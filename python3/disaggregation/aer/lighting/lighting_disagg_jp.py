...
Date - 26/11/2019
lighting disagg runs disaggregation for lighting in different modes with japan specific changes
"""
...
from python3.disaggregation.aer.lighting.functions.get_lighting_params_jp import get_lighting_params_jp
...
def lighting_disagg_jp(input_data, config, vacation_output, hist_mode, hsm_in, bypass_hsm, logger_pass):
    ...
    if not bypass_hsm:
        ...
        if config.get('MIN_NOISE_RATIO') * smoothing_noise_bound < np.nanmax(data_lighting_hsm):
    ...
    if hist_mode or bypass_hsm:
        ...
        # For Japan specifically use only lowest consumption 30% days.
        # This is to remedy the fact that HVAC generally runs there during the typical lighting hours
        # rendering the seasonal module and parameter detection ineffective

        daily_cons = np.nansum(data_filtered, axis=1)
        low_cons_days = daily_cons < percentile_1d(daily_cons,
                                                   config.get('LIGHTING_PERCENTILE_DAYS_FOR_CAPACITY_CALCULATION'))

        # Detect season in data to reject HVAC and other seasonal loads

        t_before_season_d = datetime.now()
        seasonal_flag, perc_season, smoothing_noise_bound, debug = detect_seasonal(data_filtered[low_cons_days, :],
                                                                                   num_periods, pd_mult, config,
                                                                                   logger_pass)
        ...
        cap_m, cap_e, lighting_band, debug = get_lighting_params_jp(data_season_removed[low_cons_days, :],
                                                                    smoothing_noise_bound, perc_season, num_periods,
                                                                    pd_mult, config, debug, logger_pass)
