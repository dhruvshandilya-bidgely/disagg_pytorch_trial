...
Date - 10/10/2018
Module to check hourly consistency and give initial detection of timed water heater
"""
...
def consistency_check(in_data, input_box_data, features, wh_config, debug, logger_base):
    """
    Parameters:
        in_data         (np.ndarray)    : Input 21-column data matrix
        input_box_data  (np.ndarray)    : Box data matrix
        features        (np.ndarray)    : Features matrix for the user
        wh_config       (dict)          : Configuration params for timed water heater
        debug           (dict)          : Output of all the algorithm steps
        logger_base     (dict)          : Logger object to log steps

    Returns:
        features        (np.ndarray)    : Updated user features
        debug           (dict)          : Updated debug object with algo steps
    """

    # Taking new logger base for this module

    logger_local = logger_base.get('logger').getChild('consistency_check')

    # Specific logger fo this function

    logger = logging.LoggerAdapter(logger_local, logger_base.get('logging_dict'))

    # Taking a deepcopy from raw data and box data
    ...
    raw_roll_threshold = wh_config['raw_roll_threshold']
    detection_threshold = wh_config['detection_threshold']

    max_count_bar = wh_config['max_count_bar']
    raw_count_threshold = wh_config['raw_count_threshold']
    ...
    box_end_idx = (box_energy_idx_diff[1:] < 0)
    ...
    logger.info('Max count of Start and End | {}, {}'.format(max_count_roll[0], max_count_roll[1]))
    ...
    logger.info('Std deviation of fraction | {}'.format(fraction_std))

    # Saving the raw and roll max fractions to timed debug
    ...
    logger.info('Raw by roll fraction | {}'.format(raw_roll))
    ...
    # If product of raw / roll fraction and max roll fraction below a certain number (inconsistent edges)
