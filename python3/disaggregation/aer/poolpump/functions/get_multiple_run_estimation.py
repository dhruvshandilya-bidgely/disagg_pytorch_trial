...
def get_multiple_run_estimation(input_dict, days_label, filled_days, duration_each_day, num_of_runs_each_day,
                                all_pos_edges, all_neg_edges, pp_config):
    """
    ...
    for window in range(0, num_rows, step_size):
        ...
        days_label[window:min(num_rows, window + window_size)] = 1
        filled_days[window:min(num_rows, window + window_size)] = 1
    ...
    days_filled_with_uncommon_runs = find_run_filled_days((uncontested_matrix - original_uncontested_matrix),
                                                          runs_tuple[1])
    ...
    processed_pairs = post_process_pairs(uncontested_matrix, data_clean_edges, data_bl_removed, pp_pairs, num_cols,
                                         samples_per_hour, pp_config, runs=runs_tuple[1])
    ...
    return edges_matrix, runs_tuple, consumption_matrix, structures
...
def sectional_filling(input_dict, sections, days_label, filled_days, num_of_runs_each_day, duration_each_day,
    ...
    for section in sections:
        ...
        sectional_uncontested_matrix, new_added_pairs = fill_virtual_edges(input_dict, uncontested_matrix,
                                                                           ...
                                                                           num_of_runs_each_day_copy,
                                                                           filled_days_copy,
                                                                           filled_previous_iterations,
                                                                           pp_config,
                                                                           section=section_int)
