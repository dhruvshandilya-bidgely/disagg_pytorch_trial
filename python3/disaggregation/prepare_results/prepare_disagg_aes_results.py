...
def populate_appliance_profile(disagg_input_object, disagg_output_object, empty_appliance_profile, logger, logger_pass):
    ...
    for bill_cycle_raw_idx in range(len(appliance_profile_fill_bill_cycles)):
        ...
        if valid_flag:
            ...
            logger.debug("Writing appliance profile for billcycle | %d " % bc_start)
...
def populate_lifestyle_profile(disagg_input_object, disagg_output_object, empty_lifestyle_profile, logger, logger_pass):
    ...
    lifestyle_profile_filled = disagg_output_object.get('lifestyle_profile')

    lifestyle_profile_output = copy.deepcopy(empty_lifestyle_profile)

    # Writing lifestyle profiles for all required bill cycles

    lifestyle_profile_fill_bill_cycles = disagg_input_object.get("out_bill_cycles_by_module").get("lifestyle_profile")
    ...
    for bill_cycle_raw_idx in range(len(lifestyle_profile_fill_bill_cycles)):
        ...
        valid_flag = validate_lifestyle_profile_schema_for_billcycle(disagg_output_object, bc_start, logger_pass)
...
def prepare_time_band_disagg(disagg_input_object, disagg_output_object, epoch_estimate, app_id_dict):
    ...
    epoch_hourly_idx = bill_cycle_hourly_idx + (input_data[:, Cgbdisagg.INPUT_EPOCH_IDX] -
                                                input_data[:,
                                                Cgbdisagg.INPUT_BILL_CYCLE_IDX]) // Cgbdisagg.SEC_IN_HOUR
    ...
    first_point_mod = (hourly_estimate[0, 0] - input_data[
        0, Cgbdisagg.INPUT_BILL_CYCLE_IDX]) % Cgbdisagg.SEC_IN_HOUR
...
def populate_gb_monthly_output_smb(app_id_dict, column_idx, bill_cycle_estimate, bill_cycle_idx, out_bill_cycles,
                                   bill_cycle_raw_idx, value, disagg_output_object):
    """
    Function to populate output instance of smb

    Parameters:

        app_id_dict             (dict)          : Dictionary containing appid's mappings
        column_idx              (int)           : Column identifier for appliance
        bill_cycle_estimate     (np.ndarray)    : Bill cycle level estimates of all appliances
        bill_cycle_idx          (int)           : Bill cycle identifier index
        out_bill_cycles         (np.ndarray)    : Output billing cycles
        bill_cycle_raw_idx      (int)           : Bill cycle identifier for the output billing cycle array
        value                   (float)         : Monthly consumption value of appliance
        disagg_output_object    (dict)          : Contains key attributes related to disagg output

    Returns:
        output_instance         (dict)          : Dictionary containing output instance for SMB

    """

    app_work_hour_cons = 'null'
    app_non_work_hour_cons = 'null'

    if app_id_dict[column_idx] == get_app_id('sh'):
        app_work_hour_cons = bill_cycle_estimate[bill_cycle_idx, disagg_output_object['output_write_idx_map']['sh_open']]
        app_non_work_hour_cons = bill_cycle_estimate[bill_cycle_idx, disagg_output_object['output_write_idx_map']['sh_close']]

    if app_id_dict[column_idx] == get_app_id('ac'):
        app_work_hour_cons = bill_cycle_estimate[bill_cycle_idx, disagg_output_object['output_write_idx_map']['ac_open']]
        app_non_work_hour_cons = bill_cycle_estimate[bill_cycle_idx, disagg_output_object['output_write_idx_map']['ac_close']]
    ...
    output_instance = {
        ...
        "appConsumptionAttributes": {
            "smb": {
                "workingHours": app_work_hour_cons,
                "nonWorkingHours": app_non_work_hour_cons
            }
        }
    }
...
def get_gb_monthly_output(monthly_output_maker):
    ...
    writing_for_smb = monthly_output_maker.get('writing_for_smb')
    app_id_dict = monthly_output_maker.get('app_id_dict')
    disagg_output_object = monthly_output_maker.get('disagg_output_object')
    ...
    for column_idx in range(1, num_columns):
        ...
        if writing_for_smb:
            ...
            output_instance = populate_gb_monthly_output_smb(app_id_dict, column_idx, bill_cycle_estimate,
                                                             bill_cycle_idx, out_bill_cycles, bill_cycle_raw_idx,
                                                             value, disagg_output_object)
        else:
...
def prepare_disagg_aes_results(disagg_input_object, disagg_output_object):
    ...
    logger_base = disagg_input_object.get('logger').getChild('prepare_disagg_aes_results')
    ...
    # Checking if preparing results for SMBs
    writing_for_smb = config.get('user_type').lower() == 'smb'
    writing_for_smb = writing_for_smb and (config.get('pilot_id') in PilotConstants.SMB_PILOT_LIST)
    ...
    # Initialize empty appliance and lifestyle profiles
    ...
    output_status_dict = {
        "exitCode": disagg_metrics_dict.get('aes_pipeline').get('exit_code'),
        "errorCodes": disagg_metrics_dict.get('aes_pipeline').get('error_codes'),
        "processingTime": np.round(1000 * disagg_metrics_dict.get('aes_pipeline').get('runtime'), 3),
        "gbAppStatus": []
    }
    ...
    bc_level_others, ts_level_others = calculate_others(disagg_input_object, output_write_idx_map, billing_cycle_list, epoch_estimate, bill_cycle_estimate)
    ...
    for bill_cycle_raw_idx in range(num_out_billing_cycles):
        ...
        monthly_output_maker = {'out_bill_cycles': out_bill_cycles,
                                ...
                                'writing_for_smb': writing_for_smb,
    ...
    api_disagg_output['gbTBOutput'] = prepare_time_band_disagg(disagg_input_object, disagg_output_object, epoch_estimate, app_id_dict)
...
def calculate_others(disagg_input_object, output_write_idx_map, billing_cycle_list, epoch_estimate, bill_cycle_estimate):
    ...
    smb_true_disagg_app_list = GlobalConfigParams.smb_true_disagg_app_list

    for app in smb_true_disagg_app_list:
