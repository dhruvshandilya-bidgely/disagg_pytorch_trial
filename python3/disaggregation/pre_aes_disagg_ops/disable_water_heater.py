...
Date:   14 June 2023
This function disables water heater modules for SMB pipeline based on conditions
"""
...
def disable_water_heater(disagg_input_object, logger):
    ...
    if 'wh' in module_seq and (
            smb_type.upper() not in SMBTypeConstants.WH_ENABLED_SMB_TYPES or
            pilot_id in PilotConstants.SMB_WH_DISABLED_PILOTS):

        module_seq.remove('wh')
        logger.info('Water Heater disabled | sampling rate : %.1f, pilot id : %s, smb type : %s',
