...
Author - Nisha
Date - 1/05/2022
This function disables WH in Itemization based on conditions
"""
...
def disable_hybrid_water_heater(item_input_object, logger):
    ...
    if 'wh' in module_seq and (pilot_id in PilotConstants.HYBRID_WH_DISABLED_PILOTS):
        module_seq.remove('wh')
        logger.info('Hybrid Water heater disabled | sampling rate : %.1f, pilot id : %s', sampling_rate, str(pilot_id))
