...
Author - Nisha
Date - 1/05/2022
This function disables EV in Itemization based on conditions
"""
...
def disable_hybrid_electric_vehicle(item_input_object, logger):
    ...
    if 'ev' in module_seq and (pilot_id in PilotConstants.HYBRID_EV_DISABLED_PILOTS):
        module_seq.remove('ev')
        logger.info('Hybrid Electric Vehicle disabled | sampling rate : %.1f, pilot id : %s', sampling_rate, str(pilot_id))
